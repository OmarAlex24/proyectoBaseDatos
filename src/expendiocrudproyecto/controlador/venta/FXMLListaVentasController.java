...
public class FXMLListaVentasController implements Initializable, FXMLPrincipalController.ControladorConUsuario {
  ...
  private void configurarColumnas() {
    tcFolio.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getIdVenta()).asObject());
    ...
    tcCliente.setCellValueFactory(cellData -> {
      ...
      return new SimpleStringProperty(nombreCliente != null ? nombreCliente : "Cliente general");
    });

    tcTotal.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getTotalVenta()).asObject());
  ...
  private void nuevaVenta(ActionEvent event) {
    try {
      FXMLLoader loader = new FXMLLoader(
          getClass().getResource("/expendiocrudproyecto/vista/venta/FXMLRegistroVenta.fxml"));
  ...
  private void mostrarDetallesVenta(Venta venta) {
    try {
      ...
      detalles.append("Cliente: ").append(obtenerNombreCliente(venta.getIdCliente())).append("\n");
  ...
  private String obtenerNombreCliente(int idCliente) {
    try {
      ...
      return clienteDAO.leerPorId(idCliente).getNombre();
    } catch (SQLException e) {
      // En caso de error, devolvemos el id como fallback
