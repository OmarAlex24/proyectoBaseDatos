...
public class ReporteDAO {
     ...
     */
    public List<ReporteVentasPeriodo> obtenerVentasSemanales() {
        ...
        String sql = "SELECT " +
                ...
                "WHERE v.fechaVenta >= DATE_SUB(CURDATE(), INTERVAL 8 WEEK) " +
    ...
    public List<ReporteVentasPeriodo> obtenerVentasMensuales() {
        ...
        String sql = "SELECT " +
                ...
                "MONTHNAME(v.fechaVenta) as nombreMes, " +
                "COUNT(v.idVenta) as totalVentas, " +
                "SUM(dv.total_pagado) as totalIngresos, " +
                "SUM(dv.cantidadUnitaria) as totalProductosVendidos, " +
                "AVG(dv.total_pagado) as promedioVentaDiaria " +
                "FROM venta v " +
                "INNER JOIN detalle_venta dv ON v.idVenta = dv.Venta_idVenta " +
                "WHERE v.fechaVenta >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) " +
        ...
        try (Connection conexion = ConexionBD.getInstancia().abrirConexion();
            ...
            while (rs.next()) {
                ReporteVentasPeriodo reporte = new ReporteVentasPeriodo();
                reporte.setPeriodo(rs.getString("nombreMes") + " " + rs.getInt("anio"));
                reporte.setTotalVentas(BigDecimal.valueOf(rs.getDouble("totalIngresos")));
                reporte.setTotalPedidos(rs.getInt("totalProductosVendidos"));
     ...
     */
    public List<ReporteVentasPeriodo> obtenerVentasAnuales() {
        List<ReporteVentasPeriodo> reportes = new ArrayList<>();
        String sql = "SELECT " +
                "YEAR(v.fechaVenta) as anio, " +
                "COUNT(v.idVenta) as totalVentas, " +
    ...
    public List<ReporteProductoMasVendidoCliente> obtenerProductoMasVendidoACliente(int idCliente) {
        ...
        String sql = "SELECT " +
                ...
                "p.nombre as nombreProducto, " +
                "p.precio as precioUnitario, " +
                "SUM(dv.cantidadUnitaria) as cantidadComprada, " +
                "SUM(dv.total_pagado) as totalGastadoProducto, " +
                "COUNT(v.idVenta) as numeroCompras, " +
                "AVG(dv.cantidadUnitaria) as promedioUnidadesPorCompra, " +
                "MAX(v.fechaVenta) as ultimaCompraProducto, " +
                "'RECOMENDADO PARA PROMOCIÓN' as recomendacion " +
                "FROM cliente c " +
                "INNER JOIN venta v ON c.idCliente = v.Cliente_idCliente " +
                "INNER JOIN detalle_venta dv ON v.idVenta = dv.Venta_idVenta " +
                "INNER JOIN producto p ON dv.Producto_idProducto = p.idProducto " +
                "WHERE c.idCliente = ? " +
                "GROUP BY c.idCliente, c.nombre, p.idProducto, p.nombre, p.precio " +
